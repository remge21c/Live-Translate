...
- 마이크 버튼을 끄거나 다른 화자로 전환하는 순간 마지막 발화를 강제로 번역하므로, 짧게 멈췄다가 다시 말해도 문장이 끊기지 않습니다.
- 마이크가 켜진 상태로 1.8초 이상 침묵이 감지되면 자동으로 번역이 전송되고, 화면 상단에 “자동 정지 감지” 알림이 잠시 표시됩니다.

## 레이아웃 전환

- 기기의 화면 방향(orientation)을 감지하여 세로 모드에선 기존 상/하 분할, 가로 모드에선 좌/우 분할 레이아웃을 자동 적용합니다.
- 가로 모드 기본 배치는 **왼쪽: Partner**, **오른쪽: Me**이며, 화면 하단 중앙의 “좌우 반전” 버튼으로 위치를 즉시 교체할 수 있습니다.
- 세로 모드로 돌아오면 좌우 반전 상태는 자동으로 초기화되며, 레이아웃 전환 시 번역/마이크 기능에는 영향이 없습니다.
